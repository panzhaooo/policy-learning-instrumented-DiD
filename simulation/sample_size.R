...
PCD.para2500 <- matrix(nrow = num_boot, ncol = 7)
colnames(PCD.para2500) <- c("IV.t0", "IV.t1", "IPW1", "IPW2", "Wald", "MR1", "MR2")
...
for (i in 1:num_boot) {
  ...
  PCD.para2500[i, ] <- res$PCD
}

save(PCD.para2500, file = "PCD_para_2500.RData")

PCD.para1e4 <- matrix(nrow = num_boot, ncol = 7)
colnames(PCD.para1e4) <- c("IV.t0", "IV.t1", "IPW1", "IPW2", "Wald", "MR1", "MR2")
...
for (i in 1:num_boot) {
  ...
  data <- simData(N * 2)
  res <- learn_policy(data, model = "LR")
  PCD.para1e4[i, ] <- res$PCD
}

save(PCD.para1e4, file = "PCD_para_1e4.RData")
...
PCD.ml5000 <- matrix(nrow = num_boot, ncol = 7)
colnames(PCD.ml5000) <- c("IV.t0", "IV.t1", "IPW1", "IPW2", "Wald", "MR1", "MR2")
...
for (i in 1:num_boot) {
  ...
  PCD.ml5000[i, ] <- res$PCD
}

save(PCD.ml5000, file = "PCD_ml_5000.RData")

PCD.ml2e4 <- matrix(nrow = num_boot, ncol = 7)
colnames(PCD.ml2e4) <- c("IV.t0", "IV.t1", "IPW1", "IPW2", "Wald", "MR1", "MR2")
...
save(PCD.ml2e4, file = "PCD_ml_2e4.RData")
