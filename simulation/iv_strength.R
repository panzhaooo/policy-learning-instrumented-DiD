...
simData <- function(n = 1000,
  ...
  if (iv.strength == "weak") {
    A0 <- rbinom(n, size = 1, prob = plogis(1.5 - 3 * Z + 0.2 * U0 + 2 * X1))
    A1 <- rbinom(n, size = 1, prob = plogis(-1.5 + 2 * Z - 0.15 * U1 + 1.5 * X2))
  } else if (iv.strength == "strong") {
    A0 <- rbinom(n, size = 1, prob = plogis(3 - 7 * Z + 0.2 * U0 + 2 * X1))
    A1 <- rbinom(n, size = 1, prob = plogis(-3 + 5 * Z - 0.15 * U1 + 1.5 * X2))
  }
...
PCD.para.weak <- matrix(nrow = num_boot, ncol = 7)
colnames(PCD.para.weak) <- c("IV.t0", "IV.t1", "IPW1", "IPW2", "Wald", "MR1", "MR2")

set.seed(9765)
for (i in 1:num_boot) {
  cat(i, as.character(Sys.time()), "\n")
  data <- simData(N, iv.strength = "weak")
  res <- learn_policy(data, model = "LR")
  PCD.para.weak[i, ] <- res$PCD
}

save(PCD.para.weak, file = "PCD_para_weak.RData")

PCD.para.strong <- matrix(nrow = num_boot, ncol = 7)
colnames(PCD.para.strong) <- c("IV.t0", "IV.t1", "IPW1", "IPW2", "Wald", "MR1", "MR2")

set.seed(1554)
for (i in 1:num_boot) {
  cat(i, as.character(Sys.time()), "\n")
  data <- simData(N, iv.strength = "strong")
  res <- learn_policy(data, model = "LR")
  PCD.para.strong[i, ] <- res$PCD
}

save(PCD.para.strong, file = "PCD_para_strong.RData")
...
PCD.ml.weak <- matrix(nrow = num_boot, ncol = 7)
colnames(PCD.ml.weak) <- c("IV.t0", "IV.t1", "IPW1", "IPW2", "Wald", "MR1", "MR2")
...
for (i in 1:num_boot) {
  ...
  PCD.ml.weak[i, ] <- res$PCD
}

save(PCD.ml.weak, file = "PCD_ml_weak.RData")

PCD.ml.strong <- matrix(nrow = num_boot, ncol = 7)
colnames(PCD.ml.strong) <- c("IV.t0", "IV.t1", "IPW1", "IPW2", "Wald", "MR1", "MR2")
...
for (i in 1:num_boot) {
  ...
  data <- simData(N, iv.strength = "strong")
  res <- learn_policy(data, model = "RF", V = 4)
  PCD.ml.strong[i, ] <- res$PCD
}

save(PCD.ml.strong, file = "PCD_ml_strong.RData")
